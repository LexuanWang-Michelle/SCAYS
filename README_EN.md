...
**SCAYS 1.0** is a Chinese adolescent sentiment corpus collected from Xiaohongshu (RED), currently containing **8,004** sentence-level annotated data samples with 3-class labels (Negative / Neutral / Positive), covering **17 identity keywords**.
...
|------|------|-------|------|
| Explicit Emotion (normal) | 2,172 | 1 | Contains explicit emotion words |
| Implicit Emotion (invisible) | 607 | 1 | No emotion words but implies emotion |
| Positive Emotion (positive) | 173 | 2 | Positive emotional expression |
| Neutral (neutral) | 5,052 | 0 | No emotional expression |
| **Total** | **8,004** | | |

### BERT 3-Class Model Performance
...
- **Scope**: 17 targeted keywords designed based on Dimension 1 (Identity Anchor)
- **Keywords**: 准高三, 准初三, 高三党, 初三党, 复读生, 住校生, 住宿生, 走读生, 理科生, 文科生, 美术生, 体育生, 学渣, 学霸, 卷王, 小透明, 班干部
- **Type**: Post titles (317), bodies (1,047), and comments (6,640), split at sentence-level
...
体育生,comment,体育生的快乐就是这么简单,2,Positive,Life
```
...
SCAYS/
├── data/                          # 📊 Data files
│   ├── normal.csv                 #   Explicit emotion sentences (2,172)
│   ├── invisible_labeled.csv      #   Implicit emotion annotations (5,832)
│   ├── 训练数据集.csv              #   Merged training set (8,004)
│   └── bert训练集.csv             #   BERT-ready format (raw_text + label)
├── scripts/                       # 🐍 Code scripts
│   ├── bert_3class.py             #   BERT 3-class train/predict
│   ├── xhs身份爬取1.py            #   Xiaohongshu data collection
│   ├── filter_emotion.py          #   Emotion word matching & routing
│   ├── label_invisible.py         #   Implicit emotion labeling
│   ├── merge_review.py            #   Human review merging
│   └── remove_usernames.py        #   Username anonymization
├── README.md                      # 📝 Documentation (中文)
├── README_EN.md                   #   Documentation (English)
├── SCAYS项目介绍.md                #   Project introduction
