...
**SCAYS 1.0** 是一个从小红书采集的中文青少年情感语料库，目前包含 **8,004 条** 句子级标注数据（三分类：负面 / 中性 / 正面），覆盖 **17 个身份关键词**。
...
| 隐含情绪句子 (invisible) | 607 条 | 1 | 无情绪词但有隐含情绪 |
...
- **采集范围**：基于维度 1（身份锚点）设计的 17 个定向关键词
- **关键词列表**：准高三、准初三、高三党、初三党、复读生、住校生、住宿生、走读生、理科生、文科生、美术生、体育生、学渣、学霸、卷王、小透明、班干部
- **内容类型**：帖子标题（317 条）、正文（1,047 条）、评论（6,640 条），句子级拆分
...
# 只看隐含情绪的句子
invisible = df[(df["label"] == 1) & (df["情绪类型"] == "隐含")]
...
│   ├── 训练数据集.csv              #   合并训练集 (8,004 条)
│   └── bert训练集.csv             #   BERT 可用格式 (raw_text + label)
├── scripts/                       # 🐍 代码脚本
...
```
小红书原始帖子
    ↓  DrissionPage 模拟真人浏览采集（17 个身份关键词）
帖子级数据
    ↓  句子级拆分（标题 / 正文 / 评论）
句子级语料 (8,197 条)
    ↓  去重 + 去广告 + 用户名脱敏
清洗后语料 (8,004 条)
    ↓  情绪词匹配 → 显性 / 无情绪词 分流
    ├── normal.csv (2,172 条 → label=1 负面)
    └── invisible.csv (5,832 条)
            ↓  规则引擎 + 人工审核
            ├── 隐含负面情绪 (607 条 → label=1)
            ├── 正面情绪 (173 条 → label=2)
            └── 中性 (5,052 条 → label=0)
```
