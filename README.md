...
**SCAYS 1.4** 是一个从小红书采集的中文青少年情感语料库，目前包含 **73,426 条** 句子级标注数据（三分类：负面 / 中性 / 正面），覆盖 **3 个维度、64 个关键词**（身份锚点 17 个 + 周期节律 29 个 + 关系场域 18 个）。
...
| 类别 | 数量 | 说明 |
|------|------|------|
| 显性情绪句子 (normal) | 2,172 条 | 含明确情绪词汇 |
| 隐含负面情绪 | 524 条 | 无情绪词但有隐含负面 |
| 隐含正面情绪 | 250 条 | 隐含正面表达 |
| 中性句子 | 5,052 条 | 无情绪表达 |
| 小计 | **8,004 条** | 17 个关键词 |

#### 维度 2：周期节律（Temporal Rhythm）— 25,632 条

| 类别 | 数量 | 说明 |
|------|------|------|
| 显性情绪句子 (normal) | 6,030 条 | 含明确情绪词汇 |
| 隐含负面情绪 | 1,465 条 | 无情绪词但有隐含负面 |
| 隐含正面情绪 | 586 条 | 隐含正面表达 |
| 中性句子 | 17,551 条 | 无情绪表达 |
| 小计 | **25,632 条** | 29 个关键词 |

#### 维度 3：关系场域（Relational Field）— 39,790 条

| 类别 | 数量 | 说明 |
|------|------|------|
| 显性情绪句子 (normal) | 13,470 条 | 含明确情绪词汇 |
| 隐含负面情绪 | 2,119 条 | 无情绪词但有隐含负面 |
| 隐含正面情绪 | 520 条 | 隐含正面表达 |
| 中性句子 | 23,681 条 | 无情绪表达 |
| 小计 | **39,790 条** | 18 个关键词 |
...
- **采集范围**：基于维度 1-3 设计的 64 个定向关键词
- **维度 1 关键词（17 个）**：准高三、准初三、高三党、初三党、复读生、住校生、住宿生、走读生、理科生、文科生、美术生、体育生、学渣、学霸、卷王、小透明、班干部
- **维度 2 关键词（29 个）**：月考、周测、摸底考、开学考、一模、二模、百日誓师、发成绩单、晚自习、早读、跑操、课间操、拖堂、占课、大扫除、没收手机、剪头发、查宿舍、请家长、留堂、写检讨、全校通报、补作业、赶作业、开学综合征、补课、**高考出分、高考失利、出分焦虑**
- **维度 3 关键词（18 个）**：偏心、查手机、道德绑架、重男轻女、原生家庭、爸妈吵架、留守、单亲家庭、亲子关系、被针对、班主任、老师偏心、被罚、办公室谈话、被孤立、校园冷暴力、被排挤、没有朋友
...
- [x] ~~扩大数据规模：从 8,000 条扩展到 73,000+ 条~~ ✅ 已完成（v1.4，73,426 条）
- [x] ~~维度 1-2 全部关键词完成~~ ✅ 已完成
- [x] ~~新增维度 3（关系场域）：18 个关键词，39,790 条~~ ✅ 已完成
- [ ] 继续扩展：覆盖维度 4-7 的关键词，目标 100,000+ 条
- [ ] 完善隐含情绪标注：引入 LLM 辅助 + 人工双审校，提高标注质量
- [x] ~~发布 BERT 三分类模型（验证集准确率 97%）~~ ✅ 已更新
...
│   ├── 训练数据集.csv                  #   完整训练集 (73,426 条)
│   └── bert训练集.csv                  #   BERT 格式 (raw_text + label)
├── scripts/                           # 🐍 代码脚本
│   ├── bert_3class.py                 #   BERT 三分类训练/预测
│   ├── xhs_crawl_v3.py                #   数据采集（含广告过滤）
│   ├── xhs身份爬取1.py                #   数据采集（早期版本）
...
SCAYS 的构建融合了数据高效学习领域的最新研究成果，在采集策略、标注质量和训练效率三个环节实现了系统性优化。

---

### 1. 基于重叠密度的采集策略优化

**原理**：基于 Burns et al. (OpenAI, 2023) 的弱到强泛化理论，结合 Sala et al. (ICLR 2025) 提出的重叠密度量化框架。重叠密度 = 同时具备显性情绪信号（关键词）和隐性情绪信号（情境语义）的数据占比。高重叠密度的维度采集命中率高、废料少。

分析各维度特征，**指导 v2.0 的数据扩展优先级**：

| 维度 | 重叠密度 | 采集效率 | 策略 |
|------|----------|----------|------|
| 身份锚点 | 27.0% | 高 | 优先扩展，命中率高、废料少 |
| 周期节律 | 23.6% | 中 | 次优先 |
| 关系场域 | 待测 | — | 后续补充 |

**成果**：通过重叠密度指导采集顺序，优先爬取高密度维度，以更少的采集成本获得更多有效训练数据。

---

### 2. Colander 标注质量优化

**原理**：提取 BERT 倒数两层的内部特征向量（1536 维），训练一个轻量级神经网络识别"LLM 标注与模型内部状态冲突"的模式——即使 LLM 输出高置信度，Colander 仍能从内部特征中识别出标注错误（Sala et al., NeurIPS 2024）。

对 LLM 标注进行质量审计，**发现并修正大模型的系统性标注盲区**：
...
**原理**：受 Superfiltering (Ming Li et al., ACL 2024) 启发，我们尝试用 Colander 置信度进行数据选择——只保留高置信度（标签可靠）的数据训练，去除可疑数据。

| 策略 | 训练数据量 | 准确率 | 对比全量 |
|------|-----------|--------|----------|
| 全量训练 | 58,741 条 | 97.0% | baseline |
| Colander 高置信度 50% | 29,370 条 | 90.6% | -6.4% |
| Colander 高置信度 30% | 17,622 条 | 84.3% | -12.7% |
| 随机 50%（对照） | 29,370 条 | 96.9% | -0.1% |

**关键发现**：简单去除低置信度数据反而**严重损害**模型性能（-6.4%），而随机采样 50% 几乎无损（-0.1%）。这说明低置信度数据虽然标签可能有误，但包含了模型必须学习的困难模式。**正确的做法不是丢弃它们，而是修正其标签后保留训练**——这正是 Colander 作为质量审计工具（而非剪枝工具）的正确用法。

---

### 4. 隐含情绪深度覆盖

**原理**：基于程序化弱监督（Programmatic Weak Supervision）的标注函数投票框架，设计 10 条覆盖不同语言信号的标注函数，量化"有多少情绪数据无法被任何表面模式匹配"。

- SCAYS 中 **64.9%** 的情绪数据不含任何可匹配的表面模式（关键词、句式、标点）
- 这些隐含情绪只能通过深度语义理解识别，传统关键词方法完全无法触及

**成果**：SCAYS 的核心差异化价值——**覆盖了占比 65% 的、其他数据集无法采集到的隐含情绪数据**。

---

### 5. 维度感知训练（计划中）

**原理**：受 DoReMi (Xie et al., Google, ICLR 2024) 的域感知数据混合策略启发，按维度动态调整训练权重——困难维度（重叠密度低、隐含情绪占比高）给予更高训练权重，而非所有维度均匀训练。

**预期成果**：在困难维度（如关系场域）上的准确率提升，同时不牺牲高密度维度的表现。
...
| [Xie et al., DoReMi](https://arxiv.org/abs/2305.10429) | ICLR 2024 | 维度感知训练策略 |
...
- 方法论参考：[Sprocket Lab, UW-Madison](https://sprocketlab.github.io/)（Frederic Sala）、Google Research、OpenAI
